...
cd src/tasks/501160-ButtonWithVisibilityConstraints

cd src/apps/501160-ButtonWithVisibilityConstraints

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./ButtonWithVisibilityConstraints.csproj
...
devenv /build Debug ./ButtonWithVisibilityConstraints.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./ButtonWithVisibilityConstraints.sln
...
./ButtonWithVisibilityConstraints.vsix
