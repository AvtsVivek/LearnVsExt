...
cd src/tasks/801150-IntraTextColoredAdornment

cd src/apps/801150-IntraTextColoredAdornment

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./IntraTextColoredAdornment.csproj
...
devenv /build Debug ./IntraTextColoredAdornment.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./IntraTextColoredAdornment.sln
...
./IntraTextColoredAdornment.vsix
