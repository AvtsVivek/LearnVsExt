...
cd src/tasks/351120-AddMenuButtonVsMainMenuBar

cd src/apps/351120-AddMenuButtonVsMainMenuBar

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./AddMenuButtonVsMainMenuBar.csproj
...
nuget restore ./AddMenuButtonVsMainMenuBar.sln

# The following build commands does not seem to work. 
# For debugging, build using visual studio only!!!
devenv /build Debug ./AddMenuButtonVsMainMenuBar.sln
devenv /Rebuild Debug ./AddMenuButtonVsMainMenuBar.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp 
devenv.exe /RootSuffix Exp ./AddMenuButtonVsMainMenuBar.sln
