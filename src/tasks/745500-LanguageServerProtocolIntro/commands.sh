...
cd src/tasks/745500-LanguageServerProtocolIntro

cd src/apps/745500-LanguageServerProtocolIntro

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./LanguageServerProtocolIntro.csproj
...
devenv /build Debug ./LanguageServerProtocolIntro.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./LanguageServerProtocolIntro.sln
