...
cd src/tasks/745410-ClassificationTypeRegistrySvcOldStyle

cd src/apps/745410-ClassificationTypeRegistrySvcOldStyle

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./ClassificationTypeRegistrySvcOldStyle.csproj
...
devenv /build Debug ./ClassificationTypeRegistrySvcOldStyle.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./ClassificationTypeRegistrySvcOldStyle.sln
