...
cd src/tasks/220540-NormalSpanColOpesWpfApp

cd src/apps/220540-NormalSpanColOpesWpfApp

dotnet build NormalSpanColOpesWpfApp.sln
# or
dotnet build NormalSpanColOpesWpfApp.csproj

dotnet run NormalSpanColOpesWpfApp.csproj
...
dotnet build --project ./NormalSpanColOpesWpfApp.csproj
...
nuget restore ./NormalSpanColOpesWpfApp.sln

# The following build commands does not seem to work. 
# For debugging, build using visual studio only!!!
devenv /build Debug ./NormalSpanColOpesWpfApp.sln
devenv /Rebuild Debug ./NormalSpanColOpesWpfApp.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./NormalSpanColOpesWpfApp.sln
