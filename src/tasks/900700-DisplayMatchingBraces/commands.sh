...
cd src/tasks/900700-DisplayMatchingBraces

cd src/apps/900700-DisplayMatchingBraces

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./DisplayMatchingBraces.csproj
...
devenv /build Debug ./DisplayMatchingBraces.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./DisplayMatchingBraces.sln
