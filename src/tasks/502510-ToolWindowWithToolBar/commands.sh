...
cd src/tasks/502510-ToolWindowWithToolBar

cd src/apps/502510-ToolWindowWithToolBar

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./ToolWindowWithToolBar.csproj
...
devenv /build Debug ./ToolWindowWithToolBar.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./ToolWindowWithToolBar.sln
