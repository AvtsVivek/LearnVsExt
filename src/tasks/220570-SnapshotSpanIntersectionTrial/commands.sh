...
cd src/tasks/220570-SnapshotSpanIntersectionTrial

cd src/apps/220570-SnapshotSpanIntersectionTrial

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./SnapshotSpanIntersectionTrial.csproj
...
nuget restore ./SnapshotSpanIntersectionTrial.sln

# The following build commands does not seem to work. 
# For debugging, build using visual studio only!!!
devenv /build Debug ./SnapshotSpanIntersectionTrial.sln
devenv /Rebuild Debug ./SnapshotSpanIntersectionTrial.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./SnapshotSpanIntersectionTrial.sln
...
./SnapshotSpanIntersectionTrial.vsix
