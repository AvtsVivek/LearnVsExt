...
cd src/tasks/901025-GenerateXmlSnippetFromSchema

cd src/apps/901025-GenerateXmlSnippetFromSchema

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./GenerateXmlSnippetFromSchema.csproj
...
devenv /build Debug ./GenerateXmlSnippetFromSchema.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./GenerateXmlSnippetFromSchema.sln
...
./GenerateXmlSnippetFromSchema.vsix
