...
cd src/tasks/901035-IntraTextAdornmentShowText

cd src/apps/901035-IntraTextAdornmentShowText

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./IntraTextAdornmentShowText.csproj
...
devenv /build Debug ./IntraTextAdornmentShowText.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./IntraTextAdornmentShowText.sln
...
./IntraTextAdornmentShowText.vsix
