...
cd src/tasks/501150-FirstTimeAfterExtensionIsInstalled

cd src/apps/501150-FirstTimeAfterExtensionIsInstalled

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./FirstTimeAfterExtensionIsInstalled.csproj
...
devenv /build Debug ./FirstTimeAfterExtensionIsInstalled.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./FirstTimeAfterExtensionIsInstalled.sln
...
./FirstTimeAfterExtensionIsInstalled.vsix
