...
cd src/tasks/741530-LanguagePreferencesIntro

cd src/apps/741530-LanguagePreferencesIntro

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./LanguagePreferencesIntro.csproj
...
devenv /build Debug ./LanguagePreferencesIntro.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./LanguagePreferencesIntro.sln
