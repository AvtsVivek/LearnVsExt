...
cd src/tasks/805480-SimpleStringVisualizer

cd src/apps/805480-SimpleStringVisualizer

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./SimpleStringVisualizer.csproj
...
devenv /build Debug ./SimpleStringVisualizer.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./SimpleStringVisualizer.sln
