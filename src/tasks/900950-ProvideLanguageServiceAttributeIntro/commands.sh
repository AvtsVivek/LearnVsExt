...
cd src/tasks/900950-ProvideLanguageServiceAttributeIntro

cd src/apps/900950-ProvideLanguageServiceAttributeIntro

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./ProvideLanguageServiceAttributeIntro.csproj
...
devenv /build Debug ./ProvideLanguageServiceAttributeIntro.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./ProvideLanguageServiceAttributeIntro.sln
...
./ProvideLanguageServiceAttributeIntro.vsix
