...
cd src/tasks/748540-TodoGlyphSdkContentTypeExt

cd src/apps/748540-TodoGlyphSdkContentTypeExt

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./TodoGlyphSdkContentTypeExt.csproj
...
devenv /build Debug ./TodoGlyphSdkContentTypeExt.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./TodoGlyphSdkContentTypeExt.sln
...
./TodoGlyphSdkContentTypeExt.vsix
