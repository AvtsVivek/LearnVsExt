...
cd src/tasks/502560-ActiveProcessWithToolWindow

cd src/apps/502560-ActiveProcessWithToolWindow

## I dont think we can build using the following dotnet command. 
## The project is not a dotnet core project. 

dotnet build --project ./ActiveProcessWithToolWindow.csproj
...
devenv /build Debug ./ActiveProcessWithToolWindow.sln

# If you just want to start and run visual studi in experimental mode, run the following command.
devenv.exe /RootSuffix Exp ./ActiveProcessWithToolWindow.sln
...
./ActiveProcessWithToolWindow.vsix
